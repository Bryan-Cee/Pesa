...
import { formatKes, daysRemainingInMonth, formatDateRelative } from '../utils/formatters';
...
export function Dashboard() {
  ...
  const debtPercentPaid = primaryDebt
    ? ((primaryDebt.originalBalance - primaryDebt.currentBalance) /
        primaryDebt.originalBalance) * 100
    : 0;
      ...
      >
        ...
        <View style={s.header}>
          ...
          <View style={s.headerIcons}>
            <Pressable style={s.iconBtn} onPress={() => router.push('/reminders')}>
            ...
            >
              <TabIcon name="user" color={colors.t2} size={18} />
            </Pressable>
          </View>
        </View>
        ...
        >
          <View style={s.heroCard}>
            ...
            <View style={s.heroAmountRow}>
              <View>
                ...
                <View style={s.heroAmountValueRow}>
                  ...
                  <Text style={s.heroAmount}>{totalActualFormatted}</Text>
                </View>
              </View>
              <View style={s.heroRight}>
                <Text style={s.heroBudgetAmount}>
            ...
            <View style={s.statsRow}>
              <StatChip label="Savings Rate" value={`${savingsRate}%`} color={colors.green} />
              <StatChip label="Days Left" value={String(daysRemainingInMonth())} color={colors.amber} />
              <StatChip label="Top Spend" value={topOverspend || 'None'} color={colors.coral} />
            </View>
          </View>
        </Pressable>
          ...
          <View style={s.section}>
            ...
            >
              {upcomingPayments.map((tx, i) => {
                const stripeColors = [colors.purple, colors.amber, colors.coral];
                const stripe = stripeColors[i % stripeColors.length];
                return (
                  <View key={tx.id} style={s.upcomingCard}>
                    <View style={[s.upcomingStripe, { backgroundColor: stripe }]} />
                    <View style={[s.upcomingIcon, { backgroundColor: stripe + '18' }]}>
                    ...
                    <View style={s.upcomingFooter}>
                      ...
                      <View style={[s.upcomingBadge, { backgroundColor: stripe + '18' }]}>
        ...
        {primaryDebt && debtProjection && (
          <View style={s.section}>
            <View style={s.sectionRow}>
              ...
              <Pressable onPress={() =>
                router.push({ pathname: '/debt-planner', params: { debtId: primaryDebt.id } })
              }>
                ...
                return (
                  ...
                  >
                    <View
                      style={[
                        s.pastCard,
                        { borderColor: isSurplus ? 'rgba(52,211,153,0.18)' : 'rgba(249,112,72,0.18)' },
                      ]}
                    >
                      ...
                      <View style={s.pastSparkline}>
                        <View
                          style={[
                            s.pastSparklineBar,
                            { backgroundColor: isSurplus ? colors.green : colors.red, opacity: 0.5 },
                          ]}
                        />
                      </View>
                      <Text
